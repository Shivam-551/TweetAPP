...
namespace TweetApp.Service.Services
{
    public class TweetService : ITweetService
    {
        ...
        {
            var tweetList = await _unitOfWork.Tweet.GetAllAsync();
        ...
        public async Task<IEnumerable<TweetDetailsDto>> GetTweetsByUsername(string username)
        {
            ...
            var tweetList = await _unitOfWork.Tweet.GetAllAsync(x => x.UserName == username);
        ...
        public async Task<TweetDetailsDto> PostTweet(string tweetDto,string username)
        {
            Random rn= new Random();
            username = username.Split('@')[0];
            var TweetDto = new TweetCreateDto()
            {
                id=rn.Next(),
                UserName = username,    
                TweetString = tweetDto
            };
